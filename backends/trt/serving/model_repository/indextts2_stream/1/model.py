"""Streaming (decoupled) FasterIndexTTS2 model for Triton Python Backend.

Batched streaming inference: all concurrent requests are passed as a single
batch to pipeline.generate(stream=True). Each iteration yields a BatchAudioChunk
with per-sample audio. Chunks are dispatched to each request's response_sender
...
class TritonPythonModel:
    ...
    def execute(self, requests):
        ...
        All requests run through the pipeline as a single batch in streaming mode.
        Each chunk iteration dispatches audio to each request's response_sender.
        """
        batch_size = len(requests)
        texts, spk_conds, emo_spk_conds, emo_alphas, emo_vecs = self._decode_requests(requests)

        response_senders = [req.get_response_sender() for req in requests]
        ...
        for chunk in gen:
            if batch_size == 1:
                audio = chunk.audio.astype(np.int16)
                is_last = chunk.is_last

                out_chunk = pb_utils.Tensor("audio_chunk", audio.reshape(1, -1))
                out_last = pb_utils.Tensor("is_last", np.array([[is_last]], dtype=np.bool_))
                out_sr = pb_utils.Tensor("sample_rate", np.array([[22050]], dtype=np.int32))

                response = pb_utils.InferenceResponse(output_tensors=[out_chunk, out_last, out_sr])
                if is_last:
                    response_senders[0].send(
                        response,
                        flags=pb_utils.TRITONSERVER_RESPONSE_COMPLETE_FINAL,
                    )
                    sample_finished[0] = True
                else:
                    response_senders[0].send(response)
            else:
                for b in range(batch_size):
                    ...
                    done = chunk.done_list[b]

                    if a is not None and len(a) > 0:
                        audio_data = np.asarray(a, dtype=np.int16)
                    else:
                        audio_data = np.zeros(1, dtype=np.int16)

                    out_chunk = pb_utils.Tensor("audio_chunk", audio_data.reshape(1, -1))
                    out_last = pb_utils.Tensor("is_last", np.array([[done]], dtype=np.bool_))
                    out_sr = pb_utils.Tensor("sample_rate", np.array([[22050]], dtype=np.int32))

                    response = pb_utils.InferenceResponse(output_tensors=[out_chunk, out_last, out_sr])

                    if done:
                        response_senders[b].send(
                            response,
                            flags=pb_utils.TRITONSERVER_RESPONSE_COMPLETE_FINAL,
                        )
                        sample_finished[b] = True
                    else:
                        response_senders[b].send(response)
