...
IndexTTS 生成引擎（对话 / 单轴 / 多轴导出）。

你原工程里主要有两种产出：
  1) conversation（all 轴）：run_dir/<conversation_dir_name>/<call_id>_<name>/turns/*.wav
  2) 单轴（axis mask）：
       run_dir/emotion/<label>/<call_id>_<name>/turns/*.wav
       run_dir/microexpression/<label>/<call_id>_<name>/turns/*.wav
       run_dir/speech_rate_anomaly/<label>/<call_id>_<name>/turns/*.wav
...
备注：
  - 为了避免部分环境 torchaudio 保存 wav 的 backend 问题，本文件会尝试 patch torchaudio.save。
"""
...
# -----------------------------------------------------------------------------
# PATCH torchaudio.save (optional)
# -----------------------------------------------------------------------------
...
def load_configs(cfg_dir: str) -> None:
    ...
    global EMO_ALPHA_MAP, SPEECH_RATE_CONFIG, POSTPROCESS_AUDIO_CFG
    ...
    POSTPROCESS_AUDIO_CFG = _load_json("postprocess_audio.json")
...
def calculate_energy_gain(micro: str, emo: str, rate: str) -> float:
    # 仍保持旧逻辑（你后续要上 YAML 可在 POSTPROCESS_AUDIO_CFG/micro_gain_map 做）
    base_gain = {"冷笑": 0.4, "叹息": -1.2, "倒吸气": -0.6, "哭腔": -0.8}.get(micro, 0.0)
...
def build_emo_text(
    ...
    if axis_mode == AXIS_MODE_ALL:
        ...
        crm_emo_prompt = ""
        if intention:
            crm_info = (CRM_PROMPTS or {}).get(intention, {})
    ...
    # 单轴模式默认不使用意图；保留开关（但 axis_mode_flags 已经让其为 False）
    if use_intention and intention:
        crm_info = (CRM_PROMPTS or {}).get(intention, {})
        if isinstance(crm_info, dict):
            p = crm_info.get("情绪提示", "")
            if p:
...
def extract_user_profile(obj: Dict[str, Any]) -> str:
...
def synthesize_turn(
    ...
    try:
        ...
        # scene behavior -> emo_alpha
        emo, emo_alpha = apply_scene_behavior(scene=scene, role=role, base_emo=base_emo, micro=micro, rate=rate)
        ...
        rate_cfg = get_rate_config(rate)
        max_len = int(rate_cfg.get("max_len", 28))
