...
#
# Usage:
#   bash deploy/scripts/build_image.sh --triton --fast [PRECISION]          # fast build (native Triton, default)
#   bash deploy/scripts/build_image.sh --triton [PRECISION]                 # full build (native Triton)
#   bash deploy/scripts/build_image.sh --pytriton [PRECISION]               # full build (PyTriton)
#   bash deploy/scripts/build_image.sh --export-only                        # export only
#   bash deploy/scripts/build_image.sh --engines-only [PRECISION]           # skip export
set -euo pipefail
...
if [ "$SERVING_MODE" = "triton" ]; then
    ...
    DOCKERFILE="deploy/Dockerfile.pytriton"
fi
...
if [ "$FAST_MODE" = true ]; then
    ...
    if [ "$EXPORT_ONLY" = true ]; then
        ...
        echo "  bash deploy/scripts/build_image.sh --engines-only fp16"
        exit 0
    fi
else
...
docker run --gpus all --name "faster-indextts-2-builder" \
    ...
    "$EXPORTED_IMAGE" \
    bash -c "PRECISION=${PRECISION} bash deploy/scripts/convert_checkpoint.sh && PRECISION=${PRECISION} MAX_BATCH_SIZE=${_BATCH_SIZE} bash deploy/scripts/build_engines.sh && sed -i 's/^max_batch_size:.*/max_batch_size: ${_BATCH_SIZE}/' deploy/serving/model_repository/indextts2/config.pbtxt deploy/serving/model_repository/indextts2_stream/config.pbtxt"
...
if [ "$SERVING_MODE" = "triton" ]; then
    ...
    echo "  docker run --gpus all -p 8001:8001 ${FINAL_IMAGE} python deploy/serving/triton_server.py --mode streaming --precision ${PRECISION}"
fi
echo ""
echo "To rebuild engines with a different precision (reuses exported models):"
echo "  bash deploy/scripts/build_image.sh ${SERVING_MODE:+--${SERVING_MODE} }--engines-only int8"
