...
#   PRECISION=int8 MAX_BATCH_SIZE=8 bash deploy/scripts/build_engines.sh
set -euo pipefail
...
if [ ! -d "$TLLM_CKPT_DIR" ] || [ ! -f "${TLLM_CKPT_DIR}/config.json" ]; then
    ...
    echo "Run convert_checkpoint.sh first: PRECISION=${PRECISION} bash deploy/scripts/convert_checkpoint.sh" >&2
    exit 1
fi
