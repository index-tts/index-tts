...
Usage:
    # Basic inference
    python deploy/infer.py --text "Hello world" --speaker voice.wav --output result.wav

    # Streaming (prints time-to-first-chunk)
    python deploy/infer.py --text "Hello world" --speaker voice.wav --output result.wav --stream

    # Batched inference (multiple texts, single speaker)
    python deploy/infer.py --text "First." "Second." --speaker voice.wav --output out1.wav out2.wav

    # Specify precision (auto-resolves engine dirs)
    python deploy/infer.py --text "Hello" --speaker voice.wav --output out.wav --precision fp16

    # With emotion control
    python deploy/infer.py --text "Hello" --speaker voice.wav --output out.wav \\
...
def main():
    ...
    parser.add_argument("--deploy_dir", default=None,
    ...
    deploy_dir = args.deploy_dir or os.path.join(_PROJECT_ROOT, "deploy")
    ...
    paths = resolve_engine_paths(args.precision, deploy_dir, _PROJECT_ROOT)
    ...
    if not os.path.exists(speed_emb_path):
        ...
        print("Run deploy/scripts/export_models.sh first.")
        sys.exit(1)
    if not os.path.isdir(gpt_engine_dir):
        print(f"ERROR: GPT engine dir not found at {gpt_engine_dir}")
        print(f"Run PRECISION={args.precision} bash deploy/scripts/build_engines.sh first.")
        sys.exit(1)
