"""Shared utilities for the deploy package."""

import os


def resolve_engine_paths(precision="fp16", deploy_dir=None, project_root=None):
    """Resolve TRT engine paths from precision and deploy directory.

    Args:
        precision: One of "fp32", "fp16", "int8", "int4".
        deploy_dir: Path to the deploy/ directory. Auto-detected if None.
    ...
    trt_engine_dir = os.path.join(deploy_dir, f"trt_engines_{trt_suffix}")
    gpt_engine_dir = os.path.join(deploy_dir, f"tllm_engines_{precision}")
    onnx_dir = os.path.join(deploy_dir, "onnx_models")
